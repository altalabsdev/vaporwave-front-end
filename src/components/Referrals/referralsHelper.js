...
export function getTwitterShareUrl(referralCode) {
  const message = "Trying out trading on @VWAVE_IO, up to 30x leverage on $BTC, $ETH 📈%0a%0aFor fee discounts use:";

  return `http://twitter.com/intent/tweet?text=${message}&url=https://vaporwave.farm?ref=${referralCode}`;
}
